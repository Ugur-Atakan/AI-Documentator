 ...
 */
export function assertNoDbModification(code: string): void {
  for (const pattern of DB_PATTERNS) {
    if (pattern.test(code)) {
      throw new Error(
        `Safety violation: Generated code contains database modification pattern: ${pattern.source}`
      );
    }
  }
}
 ...
 * Throws if any suspicious pattern is found.
 */
export function assertNoGitOperation(code: string): void {
  for (const pattern of GIT_PATTERNS) {
    if (pattern.test(code)) {
      throw new Error(
        `Safety violation: Generated code contains git operation pattern: ${pattern.source}`
      );
    }
  }
}
 ...
 */
export function assertOutputPathSafe(filePath: string): void {
  ...
  for (const unsafeDir of UNSAFE_DIRS) {
    if (parts.includes(unsafeDir)) {
      throw new Error(
        `Safety violation: Output path targets protected directory "${unsafeDir}": ${filePath}`
      );
    }
  }
}
 ...
 */
export function runSafetyChecks(code: string, outputPath: string): void {
