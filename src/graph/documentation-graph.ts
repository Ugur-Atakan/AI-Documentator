...
import type { ConsolidatedOutput } from "../types/controller-group.js";
 ...
 */
export async function runControllerPipeline(
  state: DocumentatorState,
  config: GraphConfig
): Promise<DocumentatorState> {
  let current = { ...state };
  ...
  try {
    ...
    const plan = await planController(
      current.controllerGroup,
      config.prismaSchema,
  ...
  try {
    ...
    const code = await writeControllerCode(
      current.documentationPlan!,
      current.controllerGroup.moduleName,
      config.prismaSchema,
      config.writerModel
    );
  ...
  if (!reviewResult.passed && current.retryCount < MAX_RETRY) {
    ...
    try {
      ...
      const retryCode = await writeControllerCode(
        current.documentationPlan!,
        current.controllerGroup.moduleName,
        config.prismaSchema,
        config.writerModel
