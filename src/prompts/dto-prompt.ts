...
export function buildDtoPrompt(
  ...
  if (endpoint.authContext.requiresContext) {
    authLines.push(
      "Controller uses @Context() decorator → workspaceId and mailboxId come from the JWT session, NOT from the request body."
    );
  }
  if (endpoint.authContext.currentUserUsages.length > 0) {
    const usages = endpoint.authContext.currentUserUsages.join(", ");
    authLines.push(
      `Controller uses @CurrentUser(${usages === "full" ? "" : `'${usages}'`}) → userId / user object comes from JWT, NOT from the request body.`
    );
  }
  if (endpoint.authContext.requiredPermission) {
    const { action, subject } = endpoint.authContext.requiredPermission;
    authLines.push(
      `CASL permission required: ${action} on ${subject} — only include fields that this permission scope would allow.`
    );
  }
  if (endpoint.authContext.requiredRoles?.length) {
    authLines.push(
      `Required roles: ${endpoint.authContext.requiredRoles.join(", ")}.`
    );
  }
