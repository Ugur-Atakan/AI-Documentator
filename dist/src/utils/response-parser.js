...
export function parseDtoResponse(raw) {
...
export function parseSwaggerResponse(raw) {
    ...
    const decoratorLines = lines.filter((l) => l.startsWith("@Api") || l.startsWith("@ApiBearerAuth"));
 ...
 */
export function parseConsolidatedResponse(raw) {
    ...
    const isEmpty = (s) => {
        ...
        if (trimmed.startsWith("// No ") || trimmed.length === 0)
            return null;
