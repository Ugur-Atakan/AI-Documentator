...
function buildDecoratorFileContent(task, controllerDecorators, requestDtoName, responseDtoName, decoratorFilePath, requestDtoPath, responseDtoPath) {
    const { endpoint } = task;
    const lines = [];
    lines.push(`import { applyDecorators } from '@nestjs/common';`);
    lines.push(`import { ApiOperation, ApiResponse, ApiBody, ApiParam, ApiQuery, ApiBearerAuth } from '@nestjs/swagger';`);
...
function writeOrDryRun(filePath, content, dryRun, skipExisting, written) {
...
export function writeGeneratedDocs(task, docs, options = {}) {
    ...
    const requestDtoName = docs.requestDtoCode
        ...
        : null;
    const responseDtoName = docs.responseDtoCode.match(/export class (\w+)/)?.[1] ?? "ResponseDto";
    const decoratorPath = path.join(decoratorDir, `${methodKebab}.decorators.ts`);
    const decoratorContent = buildDecoratorFileContent(task, docs.controllerDecorators, requestDtoName, responseDtoName, decoratorPath, requestDtoPath, responseDtoPath);
 ...
 */
export function writeConsolidatedDocs(group, output, options = {}) {
    const { outputDir, dryRun = false, skipExisting = true } = options;
