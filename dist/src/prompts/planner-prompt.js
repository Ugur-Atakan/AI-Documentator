 ...
 */
export function buildPlannerPrompt(group, prismaSchema, projectAnalysis) {
    const endpointSections = group.endpoints
        ...
        if (ep.authContext.requiredPermission) {
            authLines.push(`CASL: ${ep.authContext.requiredPermission.action} on ${ep.authContext.requiredPermission.subject}`);
        }
