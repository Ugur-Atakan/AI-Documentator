 ...
 */
export function buildSwaggerPrompt(endpoint, requestDtoCode, responseDtoCode) {
    ...
    if (authContext.isPublic) {
        ...
        authNotes.push("@ApiBearerAuth() is already on the controller class — do NOT add it again on the method.");
    }
    else {
        authNotes.push("Endpoint requires authentication. Add @ApiBearerAuth() on this method.");
    }
    if (authContext.requiresContext) {
        authNotes.push("Uses @Context() — user must have an active workspace/mailbox context set.");
    }
    if (authContext.requiredPermission) {
        const { action, subject } = authContext.requiredPermission;
        authNotes.push(`CASL permission required: ${action} on ${subject}. Include this in @ApiOperation description.`);
    }
    if (authContext.requiredRoles?.length) {
        authNotes.push(`Required roles: ${authContext.requiredRoles.join(", ")}. Mention in @ApiOperation description.`);
    }
    const hasCaslGuard = authContext.guards.some((g) => g.includes("PoliciesGuard") || g.includes("WorkspacePoliciesGuard"));
    ...
    if (existingSwagger.hasApiResponse) {
        existingNotes.push("@ApiResponse already exists on this method — only add responses that are MISSING.");
    }
    ...
    if (existingSwagger.hasApiTags) {
        existingNotes.push(`@ApiTags(${existingSwagger.apiTags.map((t) => `'${t}'`).join(", ")}) already on class — do NOT add @ApiTags() on the method.`);
    }
    // ── Determine which error responses to include ────────────────────────────
    const errorResponses = [];
    if (!authContext.isPublic) {
        errorResponses.push("401 — Unauthorized (missing or invalid Bearer token)");
    }
    if (hasCaslGuard || authContext.requiredPermission || authContext.requiredRoles?.length) {
        errorResponses.push("403 — Forbidden (insufficient permissions or missing workspace context)");
    }
