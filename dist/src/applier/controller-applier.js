 ...
 */
function controllerToKebab(controllerClass) {
 ...
 */
function extractAllExportNames(filePath) {
    try {
        const content = fs.readFileSync(filePath, "utf-8");
 ...
 */
function extractAllClassNames(filePath) {
    try {
        const content = fs.readFileSync(filePath, "utf-8");
...
function hasDecoratorImport(sourceFile, importName) {
...
function hasDecoratorOnMethod(method, decoratorName) {
    return method.getDecorators().some((d) => d.getName() === decoratorName);
}
function getBodyParam(method) {
 ...
 */
function cleanConflictingDecorators(method) {
    const removed = [];
 ...
 */
function cleanUnusedSwaggerImports(sourceFile) {
    for (const imp of sourceFile.getImportDeclarations()) {
        const moduleSpec = imp.getModuleSpecifierValue();
        if (moduleSpec !== "@nestjs/swagger")
            continue;
...
export function scanMatches(projectPath, modules) {
    const matches = [];
    ...
    for (const controllerFile of controllerFiles) {
        ...
        for (const classDecl of classes) {
            ...
            for (const method of classDecl.getMethods()) {
                ...
                let decoratorExportName = null;
                let requestDtoName = null;
                let responseDtoName = null;
                if (decFile) {
                    ...
                    if (!decoratorExportName) {
                        ...
                        decoratorExportName = allExports.find((e) => e.toLowerCase().includes(name.toLowerCase())) ?? null;
                    }
                }
                if (requestDtoFile) {
                    // Consolidated format: find the specific class for this method
                    const allClasses = extractAllClassNames(requestDtoFile);
                    requestDtoName = allClasses.find((c) => c.toLowerCase().includes(name.replace(/^(create|update|set|add)/i, "").toLowerCase()) ||
                        c.toLowerCase().includes(methodKebab.replace(/-/g, "").toLowerCase())) ?? allClasses[0] ?? null;
                }
                if (responseDtoFile) {
                    const allClasses = extractAllClassNames(responseDtoFile);
                    responseDtoName = allClasses.find((c) => c.toLowerCase().includes(name.replace(/^(get|find|list|fetch)/i, "").toLowerCase()) ||
                        c.toLowerCase().includes(methodKebab.replace(/-/g, "").toLowerCase())) ?? allClasses[0] ?? null;
                }
...
export function applyToControllers(matches, dryRun) {
    const matchesByController = new Map();
...
// -- helpers --
function findControllerFiles(projectPath, modules) {
    const srcDir = path.join(projectPath, "src");
    const files = [];
    walkDir(srcDir, files, ".controller.ts");
    if (modules && modules.length > 0) {
        return files.filter((f) => modules.some((mod) => f.toLowerCase().includes(`/${mod.toLowerCase()}/`)));
    }
    return files;
}
function walkDir(dir, result, suffix) {
