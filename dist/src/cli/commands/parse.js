...
export async function parseCommand(opts) {
    ...
    try {
        execSync(`"${tsxBin}" "${parserScript}" --project "${config.project}" > "${outputPath}"`, { stdio: ["inherit", "pipe", "inherit"] });
        const data = JSON.parse(fs.readFileSync(outputPath, "utf-8"));
        const endpoints = data.endpoints ?? [];
        console.log(chalk.green(`  ✓ ${endpoints.length} endpoints parsed`));
        console.log(chalk.dim(`  → ${outputPath}\n`));
        // Quick stats
        const publicCount = endpoints.filter((e) => e.authContext?.isPublic).length;
        const contextCount = endpoints.filter((e) => e.authContext?.requiresContext).length;
        const caslCount = endpoints.filter((e) => e.authContext?.requiredPermission).length;
        const tracedCount = endpoints.filter((e) => e.tracedService).length;
