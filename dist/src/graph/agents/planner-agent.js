 ...
 */
export async function planController(group, prismaSchema, projectAnalysis, model) {
    ...
    return withRetry(`Plan ${group.controllerClass} (${group.endpoints.length} endpoints)`, async () => {
        const response = await model.invoke(prompt);
        const raw = String(response.content);
        return parsePlanResponse(raw, group.controllerClass);
    });
}
function parsePlanResponse(raw, controllerClass) {
    ...
    throw new Error(`Could not extract documentation plan for ${controllerClass}. First 200 chars: ${raw.slice(0, 200)}`);
}
function validatePlan(parsed, controllerClass) {
    const plan = parsed;
    ...
    for (const ep of plan.endpoints) {
        if (!ep.methodName || !ep.responseDto) {
            throw new Error(`Invalid plan for ${controllerClass}: endpoint missing methodName or responseDto`);
        }
    }
    return {
        ...plan,
