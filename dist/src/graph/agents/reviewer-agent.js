 ...
 */
export function reviewGeneratedCode(output) {
    const issues = [];
    // Review request DTOs
...
function reviewDtoFile(code, fileType, issues) {
    ...
    for (const block of classBlocks) {
        ...
        for (const field of fields) {
            if (!field.decorators.includes("ApiProperty") &&
...
function reviewEnumFile(code, issues) {
...
function reviewDecoratorFile(code, issues) {
...
function extractClassBlocks(code) {
    const blocks = [];
...
function checkBalancedBraces(code, fileType, issues) {
    let depth = 0;
