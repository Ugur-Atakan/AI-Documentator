 ...
 */
export async function writeControllerCode(plan, moduleName, prismaSchema, model) {
    const prompt = buildWriterPrompt(plan, prismaSchema || null);
    return withRetry(`Write ${plan.controllerClass} code`, async () => {
        const response = await model.invoke(prompt);
        const raw = String(response.content);
        return parseWriterResponse(raw, plan.controllerClass, moduleName);
    });
}
function parseWriterResponse(raw, controllerClass, moduleName) {
...
function extractSections(raw, controllerClass) {
    ...
    if (!decorators) {
        throw new Error(`Could not extract DECORATORS section for ${controllerClass}. First 200 chars: ${raw.slice(0, 200)}`);
    }
