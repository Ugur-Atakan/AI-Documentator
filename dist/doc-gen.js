...
async function main() {
    ...
    try {
        ...
        execSync(`./node_modules/.bin/tsx "${parserScript}" --project "${args.project}" > "${args.parsedOutput}"`, { stdio: ["inherit", "pipe", "inherit"] });
    }
    catch (err) {
    ...
    if (args.modules?.length) {
        taskQueue = taskQueue.filter((t) => args.modules.some((mod) => t.endpoint.controllerFilePath.toLowerCase().includes(mod.toLowerCase())));
    }
